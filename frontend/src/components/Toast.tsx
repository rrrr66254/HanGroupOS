  ...
  return (
    <div
      className={`flex items-start gap-2.5 rounded-lg border px-3.5 py-2.5 shadow-lg backdrop-blur-sm
        text-sm text-slate-200 animate-fade-in max-w-xs w-full ${BG[toast.type]}`}
      style={{ animation: 'fadeInRight 0.25s ease' }}
    >
      {ICONS[toast.type]}
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-[12px] leading-tight">{toast.title}</p>
        {toast.body && (
